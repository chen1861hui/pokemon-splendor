...
An optional 3D-rendered GIF mode is available under Settings. When enabled, animated Pokémon artwork switches immediately from Generation V pixel sprites to PokeAPI's lightweight Showdown battle GIFs. Pixel sprites remain the default and automatic network fallback. Background, music-track, BGM volume, and sound-effect volume choices live in the same device-local settings panel; music and effects can also be enabled independently.
